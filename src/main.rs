...
fn compute_score(benchmark_time: std::time::Duration) -> u128 {
    let time_in_seconds = benchmark_time.as_secs_f64();
    if time_in_seconds > 0.0 {
        (ITERATIONS as f64 / time_in_seconds) as u128
    } else {
        0
    }
}

fn benchmark_single_thread() -> u128 {
    let start = Instant::now();
    let result = compute_range(0, ITERATIONS);
    let duration = start.elapsed();
    let score = compute_score(duration);
    ...
    println!("\x1B[32mSingle-thread score\x1B[0m: {}", score);
    return score;
}

fn benchmark_multi_thread(num_threads: usize) -> u128 {
    ...
    println!("\x1B[32mMulti-thread score\x1B[0m: {}", score);
...
async fn send_data(score_single_thread: u128, score_multi_thread: u128) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let system_info = json!({
        ...
        "score_single_thread": score_single_thread,
        "score_multi_thread": score_multi_thread,
    });
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let score_single_thread: u128 = benchmark_single_thread();
    ...
    let score_multi_thread: u128 = benchmark_multi_thread(num_threads);
    if ask_to_send() {
